...
class ConnectDb
{
    ...
    {
        $sql = "SELECT id, user_email, user_password, user_first_name, user_last_name, unique_key, site_id, pwd_status FROM op2mro9899_customers_login WHERE 
    ...
    public function user_billing_addresss($user_id)
    {
    ...
    /*public function get_shipping_charge($post_code, $site_id)
    {
        ...
        $sql = "SELECT location_name, outer_post_code, inner_post_code, delivery_charges, holiday_charges, holiday_date, site_id  FROM `op2mro9899_shipping` 
				...
				AND ( `inner_post_code` = '" . $innerCode . "' OR `inner_post_code` = '" . $InXPosts . "' OR `inner_post_code` = '" . $InnerPost . "' 
				OR `inner_post_code` = 'XXX')";
        ...
        try {
            ...
            if (count($shipping_cost) == 1)
                ...
                foreach ($shipping_cost as $s)
                    if (strlen($s->outer_post_code) == 4) return [$s];
        } catch (PDOException $e) {
            echo '{"error":{"text":' . $e->getMessage() . '}}';
            return false;
        }
    ...
    public function checkOrderIDSiteID($order_id, $siteID)
    {
        $sql_query = "SELECT * FROM op2mro9899_tmp_order WHERE order_id = :order_id AND site_id = :siteID";
        try {
            $stmt_prepare = $this->conn->prepare($sql_query);
            $stmt_prepare->bindParam("order_id", $order_id);
            $stmt_prepare->bindParam("siteID", $siteID);
            $stmt_prepare->execute();
            $orderData = $stmt_prepare->fetch(PDO::FETCH_OBJ);
            return $orderData;
        } catch (PDOException $e) {
            echo '{"error":{"text": ' . $e->getMessage() . '}}';
            return false;
    ...
    public function userBillingAddresss($user_id, $sid)
    {
        $sql_stmt = "SELECT * FROM op2mro9899_customers_billing_address WHERE user_id = :user_id AND default_address = 1 AND site_id = :sid";
        try {
            $billing_stmt = $this->conn->prepare($sql_stmt);
            $billing_stmt->bindParam("user_id", $user_id);
            $billing_stmt->bindParam("sid", $sid);
            $billing_stmt->execute();
            $billing_address = $billing_stmt->fetchAll(PDO::FETCH_OBJ);
            return $billing_address;
        } catch (PDOException $e) {
    ...
    public function getSiteHeadingBySiteId($siteid)
    {
        $sqlStmt = "SELECT * FROM op2mro9899_site_heading WHERE site_id = :siteid";
        try {
            $stmt = $this->conn->prepare($sqlStmt);
            $stmt->bindParam("siteid", $siteid);
            $stmt->execute();
            $record = $stmt->fetchAll(PDO::FETCH_OBJ);
            return $record[0]->site_heading;
        } catch (PDOException $exception) {
            echo '{"error":{"text": ' . $exception->getMessage() . '}}';
            return false;
        }
